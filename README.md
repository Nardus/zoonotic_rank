...
> [!NOTE]
> This repository uses [git-lfs](https://git-lfs.github.com/) to store pre-trained models, but they
> can more easily be obtained following the instructions [below](#ranking-novel-viruses). If you
> already have `git-lfs` installed but run into bandwidth errors when cloning, exclude the lfs
> files using:
>
> ```
>  GIT_LFS_SKIP_SMUDGE=1 git clone https://github.com/Nardus/zoonotic_rank.git
> ```
...
To download pre-trained models for predictions, run

```
./Scripts/DownloadTrainedModels.sh
```
